...
demo_row = {'state': 'NA', 'state_full': 'NA', 'contest_count': 0, 'chicken_score': 0,
            ...
            'overall_score': 0, 'best_category': 'NA', 'total_prize': 0}
...
for state in contest_states:
    ...
    new_row['total_prize'] = state_df.prize.sum()
...
sf['average_prize'] = sf['total_prize'] / sf['contest_count']

# Save the Scoring dataframe to a new file
with open('pkls/scoring_table.pkl', 'wb') as f:
    pickle.dump(sf, f)

# Write out a CSV for sharing!
sf.to_csv('kcbs_data.csv', index=False)